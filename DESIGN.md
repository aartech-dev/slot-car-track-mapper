...
Cross-checked against Ploopy's open-hardware trackball designs (`ploopyco/classic-trackball`, CERN-OHL-1.2, `hardware/Electronics/Schematics/Base-R1_005.pdf`), which wire the bare PMW3360DM-T2QU chip directly on a real shipped product. Their `MouseSensor.SchDoc` needs three separate rails — VDD 1.8V, VDDIO 3.3V (matched to host logic), and VDDPIX 1.8V fed through a 22Ω+22Ω filter with 10uF/0.1uF decoupling right at the pin — plus a 10k pull-up from CS to 3V3 so the sensor stays deselected before the MCU's GPIO drives it high. That VDDPIX filtering requirement is a concrete reason this project's plan to buy a PMW3360 *breakout* (BOM.md) rather than lay out the bare chip is the right call — a decent breakout hides that three-rail circuit behind its own onboard 1.8V regulator and exposes a single VCC. Two things worth carrying over onto our own board regardless: add the same CS-to-3V3 pull-up once a real PMW3360 breakout symbol replaces the generic connector (§8), and when that breakout is actually sourced, check whether it truly hides VDD/VDDIO/VDDPIX internally or expects them wired separately — cheap DIY-mouse-mod modules vary on this.
...
`pmw3360.py`'s driver has also been cross-checked against QMK's PMW33xx driver (`qmk/qmk_firmware`, `drivers/sensors/pmw3360.c`/`pmw33xx_common.c`), deployed on real trackball keyboards using this sensor. QMK doesn't carry the SROM byte content in-tree either (dropped for licensing, opt-in and keyboard-supplied from this same SunjunKim/PMW3360 source), so it can't confirm `SROM_FIRMWARE` byte-for-byte — but its constants do corroborate it: firmware length (4094 bytes) and product/inverse-ID signature (0x42/0xBD) both match, and its register map agrees address-for-address. Two init-sequence details this port was missing got folded in as a result: the NCS power-up toggle before `Power_Up_Reset`, and more conservative post-write settle timing — both proven across QMK's real-hardware deployments, still unverified here on this project's own board.
...
- Add a 10k pull-up from CS to 3V3 on the PMW3360 net once its real symbol replaces the generic connector, and confirm the sourced breakout's power pins (does it expose VDD/VDDIO/VDDPIX separately, or hide them behind an onboard 1.8V regulator?) — §3 addendum, prompted by cross-checking Ploopy's bare-chip reference schematic.
...
- Build a lane-combining tool (§9) — currently a documented manual procedure, not automated: no script yet translates several per-lane vector outputs into one shared-frame track file.
...
**Status:** the per-lane half of this (an accurate vector path for one lane) is implemented and validated in simulation (§6, `sim/`, `tools/`). The combining half (translating N per-lane files by their measured offsets into one shared-frame output) is currently a manual procedure as described above, not yet a script — see §8's open item. It doesn't need new sensing or firmware work, only a small offline tool once real multi-lane data exists to combine.
